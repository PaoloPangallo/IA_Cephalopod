...
def is_candidate_position_dangerous_deep(original_board, candidate_move, my_color):
    ...
    print(f"[LOOKAHEAD] Simulando mossa: ({r}, {c}, {top_face}, catture={captured})")
    ...
    for (or_, oc) in board_copy.get_empty_cells():
        ...
        if all_responses_lead_to_six:
            print(f"[LOOKAHEAD] Mossa pericolosa rilevata: ({r}, {c}, {top_face}) → tutte le risposte portano a 6 per l'avversario")
            return True

    return False


class ModSmartLookaheadStrategy6:
    ...
    def choose_move(self, board, color):
        ...
        if all(cell is None for row in board.grid for cell in row):
            ...
            print(f"[INFO] Prima mossa casuale: ({r}, {c})")
            return (r, c, 1, [])
        ...
        if safe_moves:
            ...
            print(f"[MOVE] Seleziono miglior mossa sicura: ({best_move[0]}, {best_move[1]}, {best_move[2]})")
            return best_move

        if fallback_move is None and empty_cells:
            cell = random.choice(empty_cells)
            fallback_move = (cell[0], cell[1], 1, [])
            print(f"[WARNING] Nessuna mossa sicura: fallback casuale in ({cell[0]}, {cell[1]})")
        else:
            print(f"[WARNING] Nessuna mossa sicura: fallback selezionato in ({fallback_move[0]}, {fallback_move[1]})")
